...
%
%   Some changed by DWS concerning PickableParts ('none')
    ...
    if strcmp(STYL,'speckle') || strcmp(STYL,'outspeckle')
        if any(xhatch)
            H(j) = line(xhatch,yhatch,'marker','.','linest','none', ...
                    'markersize',specksize,'color',linec, 'PickableParts', 'none');
            j = j+1;
        end
    elseif strcmp(STYL,'single') || strcmp(STYL,'cross')
        H(j) = line(xhatch,yhatch);
        set(H(j),'color',linec,'linewidth',linew, 'PickableParts', 'none');
        j = j+1;
    end
end
...
switch lower(styl),
 case 'single',
  ...
  if nargout<2,
    xi=line(xi,yi,varargin{:}, 'PickableParts', 'none');
  end;  
 case 'cross',
  [xi,yi]=drawhatch(x,y,angle,step,xsc,ysc,0);
 ...
 case 'speckle',
  ...
  if nargout<2,
    if any(xi),
      xi=line(xi,yi,'marker','.','linest','none','markersize',2,varargin{:}, 'PickableParts', 'none');
    else
      xi=NaN;
    end;    
  end; 
